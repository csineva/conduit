...
class TestRegistration:
    ...
    # test registration with valid user data from file
    @allure.id("ATC-01")
    ...
    # checking up if valid registration data is properly stored in database
    @allure.id("ATC-02")
    ...
    # test registration with already registered user data
    @allure.id("ATC-03")
    ...
    # test registration with invalid user data from file
    @allure.id("ATC-04")
...
class TestSignIn:
    ...
    def setup_method(self):
        self.page = SignInPageExtended(driver=config.get_preconfigured_chrome_driver())
        self.page.open()
    ...
    # test sign in with invalid user data from file
    @allure.id("ATC-05")
    ...
    # test sign out after sign in with valid user data
    @allure.id("ATC-06")
...
# checking the privacy policy panel
class TestPrivacyPolicy:
    ...
    def test_cookie_accept(self):
        ...
        self.page.refresh()
        # Check if cookie panel disappeared after clicking the agree button
        with pytest.raises(NoSuchElementException) as e_info:
            self.page.cookie_panel().is_displayed()


# testing the application's main page after succcessful sign in
class TestLoggedInMainPage:
    ...
    # list and save all article previews from main page to file
    @allure.id("ATC-09")
    ...
    def test_articles_list_and_write_in_file(self):
        ...
        save_articles_to_file(articles, abouts)

        # load articles back from file and compare with the main page articles
...
class TestLoggedInUserPage:
    ...
    # modify previously created article
    @allure.id("ATC-11")
